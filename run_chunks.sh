...
do
    ...
    for n in ${dataset}_baypass/snp_files/*.SNPS
    ...
    do  
        ...
        snakemake -s run_chunk_baypass.smk --cores 1 --rerun-incomplete --config chunk=$file_name_without_ext dataset=$dataset
    ...
    summary_counts=$(ls -1 ${dataset}_baypass/snp_files/*summary_betai_reg.out | wc -l)
    snps_counts=$(ls -1 ${dataset}_baypass/snp_files/*SNPS | wc -l)
    ...
    then 
        ...
        chunks=(${dataset}_baypass/snp_files/*.summary_betai_reg.out)
        first=$(chunks[0]) #this includes path 
        head -n 1 $first > ${dataset}_baypass/summary_betai_reg.out
        sed 1d ${dataset}_baypass/snp_files/*summary_betai_reg.out >> ${dataset}_baypass/summary_betai_reg.out
    else
